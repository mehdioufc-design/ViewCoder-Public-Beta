...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match ViewCoder's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole ViewCoder bar INTO the
  ...
  // DeepSeek reuses `.ds-button--primary` for dialogs and unrelated actions.
  // Resolve the control nearest the live editor instead of taking the first
  // primary button in the whole document.
  function composerControl(selector) {
    const editor = getEditor();
    if (!editor) return null;
    const candidates = [...document.querySelectorAll(selector)].filter(
      (control) =>
        !control.closest("#zs-root") &&
        control.offsetParent !== null,
    );
    let ancestor = editor;
    for (let depth = 0; depth < 14 && ancestor; depth += 1) {
      const match = candidates.find((control) => ancestor.contains(control));
  ...
  function composerFrame() {
    const ta = getEditor();
    if (!ta) return null;
    const sb = composerControl(S.sendBtn);
  ...
  // DeepSeek's resolved frame is deliberately the smallest ancestor containing
  // the editor, send button and model/mode selectors: the whole native prompt UI.
  ...
  function barMount() {
    const ta = getEditor();
    if (!ta) return null;
    const send = composerControl(S.sendBtn);
    const group = document.querySelector(S.modeRadioGroup);
  ...
  // ── Composer mode state ──────────────────────────────────────────────────
  // ViewCoder observes the user's selected model and search/deep-think controls
  // but never changes them. Model choice affects cost, speed and behavior, so it
  // must remain user-owned throughout startup and agent work.
  ...
  function enforceComposer(reason) {
    ...
    if (reason) diag("mode_observe", { reason, ...state });
    return state;
  }

  // Any user-selected DeepSeek model can drive ViewCoder. Readiness therefore
  // depends on the live composer, not a particular model or search toggle.
  async function ensureComposerReady(reason) {
    const state = enforceComposer(reason);
  ...
  // DeepSeek also uses `.ds-loading` for composer file/image uploads. Only a
  // loader attached to an assistant turn or the primary composer control is a
  // generation signal; attachment loaders must leave ViewCoder idle.
  function hasGenerationSpinner() {
    const frame = composerFrame();
    return [...document.querySelectorAll(S.generating)].some((loader) => {
      const item = loader.closest(S.chatItem);
      if (item) return isAssistantItem(item);
      const control = loader.closest(S.sendBtn);
      return !!(control && frame && frame.contains(control));
    });
  }

  // Growth-tolerant "is a generation in progress?" - the response watcher's signal.
  function isGenerating() {
    if (hasGenerationSpinner()) return true;               // spin-up spinner
    const btn = composerControl(S.sendBtn);
  ...
  function isBusyNow() {
    if (hasGenerationSpinner()) return true;
    const btn = composerControl(S.sendBtn);
  ...
  function isHardGenerating() {
    return isStopBtn(composerControl(S.sendBtn));
  }
  ...
  // DeepSeek's current code-block header contributes the visible labels
  // "json", "Copy" and "Download" to .ds-markdown.textContent. The <pre>
  // itself still contains the exact command, so expose that clean rendered
  // source to the core before it tries the combined assistant text.
  function commandSourceCandidates(item) {
    if (!item) return [];
    const candidates = [];
    const blocks = [...item.querySelectorAll(`${S.markdown} pre`)].filter(
      (block) => !block.closest(S.thinking),
    );
  ...
  async function typeAndSend(text, images) {
    ...
    if (hasImages) {
      ...
      // DeepSeek refuses the send until upload is ready. Poll for that ready state,
      // then click exactly once; repeated clicks on a stale composer can duplicate
      // an already accepted hidden result.
      const ready = await waitFor(() => {
        const btn = composerControl(S.sendBtn);
        return !!btn && !isStopBtn(btn) && btn.getAttribute("aria-disabled") !== "true";
      }, 25000);
      if (!ready) return { accepted: false, dispatched: false };
      const btn = composerControl(S.sendBtn);
      try { btn.click(); } catch { return { accepted: false, dispatched: false }; }
      const accepted = await waitFor(
        () => editorText().trim() === "" || isStopBtn(composerControl(S.stopBtn)),
        5000,
      );
    ...
    let attempted = clickSendButton();
    if (!attempted && !isBusyNow()) {
      pressEnter(editor);
      attempted = true;
    }
    if (!attempted) return { accepted: false, dispatched: false };
    const accepted = await waitFor(() => editorText().trim() === "", 3500);
    return { accepted, dispatched: true };
  }
  ...
  function stopGeneration() {
    const b = composerControl(S.stopBtn);
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "viewcoder_..." filename)
          ...
          (/^blob:/.test(im.getAttribute("src") || "") || /^viewcoder_/.test(im.getAttribute("alt") || "")));
    } catch { return []; }
  };
  ...
  async function attachImages(images) {
    ...
    // IDEMPOTENCY: one guarded provider send reuses the same images; without this
    // guard a local retry would re-attach, stacking duplicate
  ...
  //   isStarted():bool        - a ViewCoder session exists in this chat
  ...
  function installSendHooks(handlers) {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (!btn) return;
        const frame = composerFrame();
        if (!frame || !frame.contains(btn)) return;
  ...
  return {
    ...
    init({ diag: d } = {}) {
      ...
      try { document.documentElement.setAttribute("data-zs-ds-ver", "2026-08_code-block-toolbar"); } catch {}
    },
    // turns
    allItems, isUserItem, isAssistantItem, itemText, classifyText,
    assistantCount, userCount, lastAssistant, lastAssistantId, itemKey, readAssistant,
    commandSourceCandidates,
    streamLen, snapshot,
    // composer / state
