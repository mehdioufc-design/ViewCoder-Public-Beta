...
//
// Arena DOM notes (validated live, 2026-08, on the fresh `/` route):
...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. ViewCoder's
...
//  - Conversation URL is /c/<uuid>; Arena's current fresh chat is `/` (older
//    deployments used /text/direct), so both transient routes must stay supported.
// eslint-disable-next-line no-unused-vars
const ZSProvider = (() => {
  "use strict";
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  let diag = () => {}; // injected by core via init()
  // Identity of the last image set we STAGED into the composer. One provider
  // send retains the same array reference, so keying on it makes attachment
  // idempotent (see typeAndSend) - a
  ...
  // textareas are recaptcha/aria-hidden). Scope away ViewCoder's own settings
  ...
  // A genuinely fresh chat: Arena currently serves it at `/`; older deployments
  // used /text/*. Keep both transient routes while requiring an empty rendered
  // composer, so a real /c/<id> conversation can never be mistaken for startup.
  const isFreshRoute = () =>
    location.pathname === "/" || /^\/text(?:\/|$)/.test(location.pathname);
  const isFreshChat = () =>
    chatIsEmpty() && isFreshRoute() && !!getEditor();
  ...
  // Arena's rounded anchor is the visible prompt card inside the broader form.
  const coverTarget = () => barAnchor() || composerFrame() || getEditor();
  ...
  const allButtons = () => [
    ...(composerFrame()?.querySelectorAll("button") || []),
  ];
  const sendButton = () =>
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…ViewCoder: result truncated to fit Arena's input limit - ` +
  ...
  async function typeAndSend(text, images) {
    ...
    const ready1 = await waitFor(sendReady, 20000);
    diag("arena.tas.ready", { ready: ready1 });
    if (!ready1) return { accepted: false, dispatched: false };
    // Attach LAST, right before the send - see gemini.js for why. Guard a local
    // send retry (same `images` array ref): skip when
    ...
    // Dispatch exactly once. An ambiguous acknowledgement is owned by the core's
    // post-dispatch wait and must never become a second copy in the conversation.
    ...
    const sent = await waitFor(
      () => editorText().trim() === "" || isHardGenerating(),
      1500,
    );
  ...
  // ViewCoder only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // Visible mode guard for the ViewCoder bar (core renderBar reads this every
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `ViewCoder only works in plain chat. ${um.label} mode uses a different ` +
    ...
    return `Switch the mode dropdown to <b>Direct</b> - ViewCoder only works in ` +
      `Direct mode (current: <b>${safeName}</b>).`;
  }
  ...
  // ViewCoder bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored ViewCoder bar (a real, full-width element hugging the
  ...
  // being (re)attached within one provider send (same array = same id).
  let _imgSeq = 0;
  ...
  // `/` and /text/* are fresh chats with no conversation id yet → "" (transient,
  // never persisted as "started"). A real conversation is /c/<uuid>.
  const conversationKey = () => (isFreshRoute() ? "" : location.pathname);
  ...
  return {
    ...
    reliableCounts: true,
    // Observe the user's Arena mode without opening or changing its selector.
    // Unsupported modes remain visibly gated by modeWarning().
