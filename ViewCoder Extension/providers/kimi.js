...
const ZSProvider = (() => {
  ...
  const S = {
    ...
    // the ViewCoder command blocks, so a session must not be started while it
    // is on.
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed ViewCoder bar over the editor. The real
  ...
  // Kimi's generic composerFrame can be page-wide; .chat-editor is the precise
  // rounded card containing its editor, model controls, attachments and send.
  const coverTarget = () => barAnchor() || gateTarget() || getEditor();
  ...
  const sendControl = () => {
    ...
    return (c && c.querySelector(S.sendBtn)) || null;
  };
  ...
  async function typeAndSend(text, images) {
    ...
    try {
      ...
      // Only attach if nothing is pending yet; a provider-local retry must not
      // upload another duplicate copy.
      ...
      const accepted = await waitFor(() => editorText().trim() === "", 4000);
  ...
  // favor its own built-in tools over the ViewCoder command protocol - check
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Turn off Kimi's own <b>Agent</b> mode (next to the message box) - ` +
        `it replaces the ViewCoder commands with Kimi's native tools and breaks the agent loop.`;
    return "";
  }
  ...
  // composer's top edge) sits ON TOP of the mask and the provider notice floats
  ...
  // ViewCoder never opens, closes, or changes Kimi's model picker. The selected
  // model belongs entirely to the user; composer enforcement only checks that
  ...
  function installSendHooks(handlers) {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (!frame || !frame.contains(ctrl)) return;
  ...
  return {
    ...
    // the capture is shown in the core's left-hand ViewCoder popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    // runner, etc.) instead of emitting the ViewCoder command blocks that drive
    // Roblox Studio - model behavior, not something the prompt fully prevents.
    unstableWarning:
      "Kimi may choose its built-in tools instead of the connected Studio tools. " +
      "If Studio activity pauses, ask it to continue through ViewCoder or begin a fresh chat.",
    noticeLabel: "Notice",
