...
const ZSProvider = (() => {
  ...
  const S = {
    ...
    inputArea: "input-area-v2, input-area, [class*='input-area']",
    codeWrap: "code-block",
  ...
  // Scope to the SITE's composer only: skip ViewCoder's own injected UI (the
  ...
  // ViewCoder bar stuck on "Agent active" with no way to start a new session.
  const isFreshChat = () => chatIsEmpty() && !!getEditor();

  // Resolve only Gemini's visible, live composer; home-page templates otherwise
  // make the shared placer fall back to a wide floating bar.
  const visibleComposer = (el) => {
    if (!el || !el.isConnected || el.closest("#zs-root")) return false;
    const r = el.getBoundingClientRect();
    const s = getComputedStyle(el);
    return r.width > 0 && r.height > 0 && s.display !== "none" && s.visibility !== "hidden";
  };
  const composerFrame = () => {
    for (const selector of [S.inputArea, "input-area-v2", "input-area", "fieldset.input-area-container"]) {
      for (const el of document.querySelectorAll(selector)) if (visibleComposer(el)) return el;
    }
    return null;
  };
  // input-area-v2 is Gemini's complete rounded prompt card: editor, toolbox,
  // attachments and primary send/stop control.
  const coverTarget = () => composerFrame() || getEditor();
  ...
  function barMount() {
    ...
    if (!ia) return null;
    const col = ia.matches("fieldset.input-area-container")
      ? ia
      : (ia.closest("fieldset.input-area-container, .input-area-container, form") || ia.parentElement);
    if (!col || !visibleComposer(col)) return null;
    const before = col.querySelector("input-area-v2, input-area") || ia;
    return { parent: col, before };
  }
  ...
  const findButtonBySignal = (pattern) => actionButtons().find((button) => pattern.test(buttonSignal(button))) || null;
  const sendButton = () => findButtonBySignal(/\barrow_upward\b|\bsend\b|\bsubmit\b/);
  const stopButton = () => findButtonBySignal(/\bstop\b|\bcancel response\b/);
  ...
  // Gemini may pause without changing visible text while a tool executes.
  // Treat its real stop control as live for the requested 1m45s before deciding
  // it is stale; otherwise ViewCoder can report a false timeout mid-command.
  const WEDGE_MS = 105000;
  let _stopSince = 0;
  function genActive() {
    sampleStream();
  ...
  // Gemini's Imagen surface is not represented by the ordinary response stop
  // icon for its entire lifetime. Detect its own progress/status elements so
  // the core waits for the completed native image instead of writing a tool
  // result while Gemini is still painting it.
  function nativeImageGenerationState(candidate = lastAssistant()) {
    const item = candidate?.closest?.(S.assistantItem) || candidate;
    if (!item || item.closest?.("#zs-root")) return { active: false, finished: false };
    const text = textWithout(item, ".zs-chip, #zs-root").replace(/\s+/g, " ").trim();
    const busy = !!item.querySelector(
      'mat-progress-spinner, [role="progressbar"], [aria-busy="true"], [class*="loading-shimmer" i], [class*="image-generation" i][class*="loading" i], [data-test-id*="image" i][data-state="loading"], [data-testid*="image" i][data-state="loading"]',
    );
    const status = /\b(?:creating|generating|rendering|drawing)\s+(?:your\s+|an?\s+)?image\b|\b(?:preparing\s+visual\s+context|sketching\s+it\s+out|imagen\s+is\s+working|working\s+on\s+it|refining\s+details|finishing\s+up|polishing\s+details|adding\s+final\s+touches|almost\s+there)\b/i.test(text);
    const finishedImage = [...item.querySelectorAll("img")].some((image) => {
      if (image.closest?.("#zs-root") || image.complete !== true) return false;
      if (Number(image.naturalWidth || 0) < 96 || Number(image.naturalHeight || 0) < 96) return false;
      const alt = image.getAttribute("alt") || "";
      return /generated|imagen/i.test(alt) || !!image.closest(
        '[class*="generated-image" i], [class*="image-generation" i], [data-test-id*="generated-image" i], [data-testid*="generated-image" i]',
      );
    });
    return {
      active: !finishedImage && (busy || status),
      finished: finishedImage && !busy && !status,
      root: item,
    };
  }
  ...
  async function typeAndSend(text, images) {
    ...
    try {
      // Keep the provider-local send path idempotent. Naively redoing both steps
      // after a delayed acknowledgement is destructive: (1) setEditorText's
      // select-all + insertText wipes the
      ...
      // (validated live: repeated attempts produced separate attach.dispatchPaste
      ...
      const accepted = await waitFor(() => editorText().trim() === "", 4500);
  ...
  function fileFromImage(img, i) {
    ...
    return new File([arr], `viewcoder_${Date.now()}_${i}.${ext}`, { type: mime });
  }
  ...
  return {
    ...
    // Emergency editor-fallback geometry. The normal full-card coverTarget above
    // fits input-area-v2 exactly and therefore ignores these values.
    coverPad: 8,
    coverOffsetY: -6,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the ViewCoder panel.
    ...
    unstableWarning:
      "Long Gemini chats can gradually favor normal replies over Studio actions. " +
      "If that happens, ask Gemini to continue with the available ViewCoder tools or begin a fresh chat.",
    noticeLabel: "Notice",
    ...
    // composer / state
    getEditor, editorText, chatIsEmpty, isFreshChat, composerFrame, barMount, coverTarget,
