...
// that carry ViewCoder commands) in a Monaco editor that DISPOSES off-screen
...
(() => {
  ...
  // Streaming responses can produce hundreds of network chunks per second.
  // Writing the complete accumulated reply into the DOM for each chunk caused
  // quadratic serialization work and woke the isolated-world observer just as
  // often. Coalesce partial frames, but flush terminal state immediately.
  const publish = (obj, immediate = false) => {
    pendingPublish = obj;
    if (immediate) {
  ...
  async function consume(resp) {
    ...
    acc.done = true;
    publish(
      { rid: acc.rid, text: acc.text, done: acc.done, t: Date.now() },
      true,
    );
  }
