...
const ZSProvider = (() => {
  ...
  function ensureCodeObserver() {
    ...
    // Snapshot only code blocks touched by the current mutation batch. The old
    // implementation re-scanned every code block in the whole conversation on
    // every streamed character, so its work grew with both response length and
    // chat length. Monaco only invalidates the block containing the mutation (or
    // a newly inserted subtree), so a small affected-block set is sufficient.
    const affectedBlocks = (mutations) => {
      ...
      const addClosest = (node) => {
        const element =
          node?.nodeType === Node.ELEMENT_NODE
            ...
            : node?.parentElement;
        if (!element) return;
        const closest = element.matches?.(S.codeWrap)
          ? element
          : element.closest?.(S.codeWrap);
        if (closest) blocks.add(closest);
        element.querySelectorAll?.(S.codeWrap).forEach((block) =>
          blocks.add(block));
      };
      for (const mutation of mutations) {
        addClosest(mutation.target);
        mutation.addedNodes?.forEach(addClosest);
      }
      return blocks;
    };
    let _snCount = 0, _snMaxMs = 0, _snMaxBlocks = 0, _snWinStart = Date.now();
    const snapshotChanged = (mutations) => {
      const t0 = (self.performance || Date).now();
      const blocks = affectedBlocks(mutations);
      blocks.forEach(snapshotCode);
      const ms = (self.performance || Date).now() - t0;
      _snCount++;
      if (ms > _snMaxMs) _snMaxMs = ms;
      if (blocks.size > _snMaxBlocks) _snMaxBlocks = blocks.size;
      const now = Date.now();
      if (now - _snWinStart >= 1000) {
        if (_snCount > 120 || _snMaxMs > 25 || _snMaxBlocks > 8) {
          diag("code.snapshotChanged", {
            firesPerSec: _snCount,
            blocks: _snMaxBlocks,
            maxMs: Math.round(_snMaxMs),
          });
        }
    ...
    _codeObs = new MutationObserver(snapshotChanged);
    try {
  ...
  // Scope to the site's composer only; skip ViewCoder's own injected textarea
  ...
  // Use the real message-input wrapper, not barAnchor's possible zero-height
  // fixed-position shell, so the cover fits Qwen's full prompt card exactly.
  ...
  const sendButton = () => {
    ...
    const scope =
      frame?.querySelector(".message-input-right-button-send") ||
      frame?.querySelector(".chat-prompt-send-button");
    const btn =
      (scope ? scope.querySelector(S.sendBtn) : null) ||
      frame?.querySelector(S.sendBtn) ||
      null;
    if (!btn) return null;
  ...
  const stopButton = () =>
    composerFrame()?.querySelector(S.stopBtn) || null;
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…ViewCoder: result truncated to fit Qwen's ${SEND_CAP}-character input ` +
  ...
  async function typeAndSend(text, images) {
    ...
    try {
      ...
      // Guard a provider-local retry: only attach if nothing is staged yet,
      // otherwise another attempt would paste a duplicate copy.
      ...
      const sent = await waitFor(() => editorText().trim() === "", 4000);
      // Only retire the tapped response after Qwen accepted the next turn. Doing
      // this before the click meant a failed send discarded the response we still
      // needed to process and left the loop looking idle.
      if (dispatched) rememberSentResponse();
      return { accepted: sent, dispatched };
    } finally {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        const stopped =
          e.target &&
          e.target.closest &&
          e.target.closest(S.stopBtn);
        if (stopped) {
          const frame = composerFrame();
          if (frame && frame.contains(stopped)) {
  ...
  // NOTE: a provider notice used to be injected next to Qwen's "Auto"/"Think"
  // thinking-modes, on the theory that those modes fired the model's OWN native
  // tool-calls instead of ViewCoder commands. Removed 2026-07: the real cause of
