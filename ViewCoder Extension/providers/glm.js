...
const ZSProvider = (() => {
  ...
  const S = {
    ...
    // so "viewcoder_….jpg JPG 179 KB Output of 'screen_capture'…" failed to match
  ...
  // The rounded flex-column is GLM's complete editor + toolbar prompt card.
  ...
  async function typeAndSend(text, images) {
    ...
    // Only attach if nothing is staged yet; a provider-local retry must not
    ...
    // click no-opped and the message remained in the input (the user's "message
    // sits in the input then sends after a long time"). We now wait
  ...
  return {
    ...
    // Shown as a permanent, non-intrusive provider notice in the bar. The
    ...
    unstableWarning:
      "GLM may return no reply while z.ai is at capacity. This comes from the provider, " +
      "not the Studio bridge. Try the GLM-5.2 model or retry the request shortly.",
    noticeLabel: "Notice",
